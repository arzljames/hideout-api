...
- `user:<id>`: `invite:received`, `invite:revoked`, `member:removed`, `session:expired`
...
- `room_bans` (room_id, steam_id, banned_by, reason, created_at) PK(room_id, steam_id): prepared, not yet enforced. The bans feature will add the ban/unban API and make `redeem_invite_link`, `respond_to_direct_invite`, and `create_direct_invite` refuse banned SteamIDs.
...
- Multi-step writes are Postgres functions (service role only): `create_login_session`, `create_room`, `update_room`, `redeem_invite_link`, `respond_to_direct_invite`, `transfer_ownership`, `delete_room`, `remove_member`, `change_role`, `create_channel`, `rename_channel`, `reorder_channels`, `delete_channel`, `send_message`, `edit_message`, `delete_message`, `create_link_invite`, `create_direct_invite`, `revoke_invite`. Their error SQLSTATEs map to HTTP in the migration header (`HX001` → 404 for non-members and missing rooms, `HX002` → 403 for members lacking the role; for channels, `HX006` → 409 `CHANNEL_LIMIT_REACHED`, `HX007` → 409 `CHANNEL_ORDER_STALE` for a stale reorder list, `HX008` → 409 `LAST_TEXT_CHANNEL`, and `23505` → 409 `CHANNEL_NAME_TAKEN`; for messages, `HX009` → 409 `CHANNEL_NOT_TEXT` and `HX010` → 409 `IDEMPOTENCY_KEY_REUSED`; `HX005` → 409 `OWNER_PROTECTED` when the owner would leave or be removed; for invites, `HX011` → 409 `ALREADY_MEMBER` and `HX012` → 409 `INVITE_ALREADY_PENDING`). Realtime access is `private.can_access_topic(topic)`, the only Hideout function `authenticated` can execute.
