...
- Rate limits: auth 10/min/IP (separately for `/api/auth/steam` and the callback; both redirect with `?auth_error=RATE_LIMITED` instead of a JSON 429, since they are browser navigations), realtime-token 30/hour/user, messages 10/10s/user (send, edit, delete), message reads 120/min/user, invite create 20/hour/user, invite redeem 10/min/user, room create 10/hour/user, room update 30/hour/user, channel writes 60/hour/user.
