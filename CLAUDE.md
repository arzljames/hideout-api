...
- `user:<id>`: `invite:received`, `invite:revoked`, `member:removed` (with `banned: true` when the removal was a ban), `session:expired`
...
- **Revocation:** Realtime checks policies when a browser joins a channel and when it sends a refreshed token, not per message. On removal or ban (or room deletion), Node broadcasts `member:removed` to `user:<id>` and an honest client leaves; a removed member fails the policy on their next join or token refresh. A client that never refreshes keeps access until its current token's `exp` (15 minutes at most). Tokens are per user, not per room. Confirm current Supabase behavior before relying on anything stronger.
...
- Who invites: link invites are owner/admin only (a plain member gets 403); direct invites are open to any member.
- `room_bans` (room_id, steam_id, banned_by, reason, created_at) PK(room_id, steam_id): permanent until lifted, keyed by SteamID64 (survives a profile delete). The owner bans anyone but themself, an admin bans plain members (`ban_member` removes the member like `remove_member`); the owner or any admin lifts any ban (`unban`). Enforced: `redeem_invite_link` and `respond_to_direct_invite` (accept only) return status `banned` (→ 403 `BANNED`), and `create_direct_invite` raises `HX013`.
...
- Multi-step writes are Postgres functions (service role only): `create_login_session`, `create_room`, `update_room`, `redeem_invite_link`, `respond_to_direct_invite`, `transfer_ownership`, `delete_room`, `remove_member`, `change_role`, `create_channel`, `rename_channel`, `reorder_channels`, `delete_channel`, `send_message`, `edit_message`, `delete_message`, `create_link_invite`, `create_direct_invite`, `revoke_invite`, `ban_member`, `unban`. Their error SQLSTATEs map to HTTP in the migration header (`HX001` → 404 for non-members and missing rooms, `HX002` → 403 for members lacking the role; for channels, `HX006` → 409 `CHANNEL_LIMIT_REACHED`, `HX007` → 409 `CHANNEL_ORDER_STALE` for a stale reorder list, `HX008` → 409 `LAST_TEXT_CHANNEL`, and `23505` → 409 `CHANNEL_NAME_TAKEN`; for messages, `HX009` → 409 `CHANNEL_NOT_TEXT` and `HX010` → 409 `IDEMPOTENCY_KEY_REUSED`; `HX005` → 409 `OWNER_PROTECTED` when the owner would leave or be removed; for invites, `HX011` → 409 `ALREADY_MEMBER` and `HX012` → 409 `INVITE_ALREADY_PENDING`; for bans, `HX013` → 409 `USER_BANNED` and `HX003` from `unban` → 404 when the SteamID isn't banned). Realtime access is `private.can_access_topic(topic)`, the only Hideout function `authenticated` can execute.
...
- Send message accepts `Idempotency-Key`. Redeem, accept, and decline invite are idempotent by design (a repeat returns `already_member` or 204), so they need no key.

## Security rules

- `requireAuth` everywhere except `/api/auth/steam*`, `/api/health`, `/api/ready`, `/api/contract/*`, `/api/docs*`, `GET /api/invites/:token/preview`, and `POST /api/livekit/webhook` (authenticated by its LiveKit signature instead; mounted in `app.ts` before the JSON parser and the Origin check).
...
- LiveKit tokens: identity = profile id, room = `voice_<channelId>`, microphone only, valid 60s for the initial connect only (LiveKit refreshes tokens for connected participants). Removal (leave, remove, ban, channel or room delete) calls `removeParticipant` with LiveKit's default revocation (tokens with `nbf` before now + 1 min leeway stop working), retried; so a re-invited member may wait about a minute to rejoin voice. Backstops: the webhook kicks non-members on `participant_joined` (503 if membership can't be checked, so LiveKit retries), and every participant list read or broadcast kicks connected non-members. Webhooks verified with `WebhookReceiver` on the raw body; the webhook URL is `API_URL/api/livekit/webhook`, configured in LiveKit Cloud (local dev needs a tunnel to receive webhooks).
- Rate limits: auth 10/min/IP (separately for `/api/auth/steam` and the callback; both redirect with `?auth_error=RATE_LIMITED` instead of a JSON 429, since they are browser navigations), realtime-token 30/hour/user, messages 10/10s/user (send, edit, delete), message reads 120/min/user, invite create 20/hour/user, invite redeem 10/min/user (shared by redeem, accept, decline), invite preview 60/min/IP, invite reads (room list + inbox) 60/min/user, invite revoke 60/hour/user, room create 10/hour/user, room update 30/hour/user, channel writes 60/hour/user, member writes 30/hour/user (leave, remove, change role, transfer ownership, ban, unban), ban list 60/min/user, voice token 20/min/user, voice participant reads 60/min/user, LiveKit webhook 600/min/IP.
